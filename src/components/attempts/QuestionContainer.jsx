...
export default function QuestionContainer(props) {
    ...
    function getMarks() {
        axios.get('http://localhost:5000/api/getSum/' + atm_id).then((res) => {
            console.log('<><>'+res.data.data[0].marks)
            setSum(res.data.data[0].marks);
            console.log('<><><>'+sum)
    ...
    const initiateAttempt = () => {
        ...
        axios.post('http://localhost:5000/api/newAttempt', quest).then((res) => {
            console.log("attempt initialized!",res.data.id);
            setAtId(res.data.id);
    ...
    return (
        <>
            <div className="container" >
                ...
                {is_student === "1" && sum > 0 ?
                    <>
                        <a href={`/studyMaterials?sub=${path}&attempt=${atm_id}`}><button style={{ marginTop: "10px" }} class="btn btn-primary">Improve</button></a>
                    </>
                    :
                    <>
